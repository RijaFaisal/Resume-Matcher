...
def mock_dependencies():
    with patch("src.api.app.client") as mock_groq, patch.dict(
        "src.api.app.state", clear=True
    ) as mock_state:
        ...
        # Mock State
        mock_state["policy_engine"] = (
            None  # Disable guardrails for logic simplification
        )
        ...
        mock_state["faiss_index"].search.return_value = (
            np.array([[0.9]]),
            np.array([[0]]),
        )
...
def test_ask_endpoint_success(mock_dependencies):
    ...
    response = client.post(
        "/ask", json={"question": "How to improve?", "user_context": "Junior Dev"}
    )
    ...
    response_pdf = client.post("/ask", json={"question": "Make me a resume"})
