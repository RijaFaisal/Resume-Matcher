...
class TestHallucinationDetector:
    ...
    def test_grounding_check(self):
        ...
        assert (
            len(result.violations) > 0 or result.confidence_scores["confidence"] < 0.8
        )
...
class TestPolicyEngine:
    ...
    def setup_method(self):
        ...
        self.balanced_engine = PolicyEngine(GuardrailsConfig(mode=PolicyMode.BALANCED))
        self.strict_engine = PolicyEngine(GuardrailsConfig(mode=PolicyMode.STRICT))
    ...
    def test_valid_input_all_modes(self):
        ...
        for engine in [
            self.permissive_engine,
            self.balanced_engine,
            self.strict_engine,
        ]:
...
class TestIntegrationScenarios:
    ...
    def test_resume_submission_flow(self):
        ...
        match_output = (
            "Top 3 matching jobs: 1) ML Engineer 2) Python Developer 3) Data Scientist"
        )
