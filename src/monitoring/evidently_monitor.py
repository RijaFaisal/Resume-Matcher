...
class EvidentlyMonitor:
    ...
    def create_drift_report(
        self, reference_window: int = 1000, current_window: int = 100
    ) -> Optional[str]:
        """
    ...
    def create_performance_report(self, window: int = 1000) -> Optional[str]:
        """
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "avg_latency_ms": (
                df["total_latency_ms"].mean() if "total_latency_ms" in df else 0
            ),
            "p95_latency_ms": (
                df["total_latency_ms"].quantile(0.95) if "total_latency_ms" in df else 0
            ),
            "p99_latency_ms": (
                df["total_latency_ms"].quantile(0.99) if "total_latency_ms" in df else 0
            ),
    ...
    def analyze_cost_trends(self, window: int = 24) -> Dict:
        ...
        return {
            ...
            "cost_trend": (
                "increasing" if hourly_cost.is_monotonic_increasing else "stable"
            ),
        }

    def detect_anomalies(
        self, metric: str = "total_latency_ms", threshold_std: float = 3.0
    ) -> List[Dict]:
        """
