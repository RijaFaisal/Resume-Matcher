...
class PIIDetector:
    ...
    def detect(self, text: str) -> ValidationResult:
        ...
        logger.info(f"PII Detection: {len(violations)} violation(s) found, risk={risk_level.value}")
    ...
    def validate(self, text: str) -> ValidationResult:
        ...
        if self.enable_pii_detection:
            ...
            if pii_result.risk_level.value != "safe":
                highest_risk = max(highest_risk, pii_result.risk_level, key=lambda x: list(RiskLevel).index(x))
        ...
        if self.enable_injection_filter:
            ...
            if injection_result.risk_level.value != "safe":
                highest_risk = max(highest_risk, injection_result.risk_level, key=lambda x: list(RiskLevel).index(x))
