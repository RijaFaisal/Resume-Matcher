    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "violation_rate": round(
                (self.input_violations + self.output_violations)
...
class PolicyEngine:
    ...
    def validate_input(self, user_input: str) -> GuardrailResult:
        ...
        try:
            ...
            if self.config.log_violations and validation_result.violations:
                ...
                logger.debug(
                    f"Input validation passed: risk={validation_result.risk_level.value}"
                )
            ...
            if self.config.block_on_error:
                return GuardrailResult(
                    allowed=False, metadata={"error": str(e), "blocked_on_error": True}
                )
            else:
                return GuardrailResult(
                    allowed=True,
    ...
    def moderate_output(
        self, llm_output: str, context: Optional[str] = None
    ) -> GuardrailResult:
        """
        ...
        try:
            ...
            if self.config.log_violations and moderation_result.violations:
                ...
                logger.debug(
                    f"Output moderation passed: action={moderation_result.action.value}"
                )
