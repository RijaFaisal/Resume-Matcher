...
class ToxicityFilter:
    ...
    def __init__(self, toxicity_threshold: float = 0.7):
        ...
        self.toxic_keywords = {
            ...
            "hate_speech": [
                "hate", "racist", "nazi", "supremacist", "kill all",
                "exterminate", "subhuman", "inferior race"
            ],
            "sexual": [
            ...
            "violence": [
                "kill", "murder", "shoot", "stab", "torture", "assault",
                "bomb", "weapon", "suicide", "self-harm"
            ],
            "discrimination": [
                "discriminate", "prejudice", "bigot", "stereotype",
                "sexist", "ageist", "homophobic"
            ],
        }
    ...
    def detect(self, text: str) -> ModerationResult:
        ...
        logger.info(f"Toxicity Detection: score={toxicity_score:.2f}, action={action.value}")
...
class HallucinationDetector:
    ...
    def detect(self, output: str, context: Optional[str] = None) -> ModerationResult:
        ...
        if hallucination_count > 0:
            violations.append(f"Detected {hallucination_count} hallucination indicator(s)")
        ...
        logger.info(f"Hallucination Detection: confidence={confidence_score:.2f}, action={action.value}")
...
class OutputModerator:
    ...
    def __init__(
        ...
        if enable_hallucination_detector:
            self.hallucination_detector = HallucinationDetector(confidence_threshold=confidence_threshold)
