...
async def lifespan(app: FastAPI):
    ...
    try:
        if os.path.exists(CONFIG.JOB_EMBEDDINGS_PATH):
            ...
            logger.warning(
                f"⚠️ Job embeddings not found at {CONFIG.JOB_EMBEDDINGS_PATH}"
            )
...
def generate_pdf_resume(data: Dict) -> str:
    ...
    try:
        ...
        pdf.cell(
            0,
            10,
            data.get("name", "Name Placeholder"),
            new_x="LMARGIN",
            new_y="NEXT",
            align="C",
        )
...
def match_resume(request: MatchRequest):
    ...
    if not state["sbert_model"] or state["job_embeddings"] is None:
        raise HTTPException(
            status_code=503,
            detail="Matcher services not ready (model/embeddings missing)",
        )
    ...
    try:
        ...
        resume_embedding = state["sbert_model"].encode(
            text_to_process, convert_to_tensor=True
        )
...
def ask(payload: Query):
    ...
    try:
        ...
        if state["policy_engine"]:
            ...
            if not val.allowed:
                violations = (
                    val.input_validation.violations if val.input_validation else []
                )
                return {
            ...
            if "generate_resume" in clean_answer[:100]:  # Hint check
                ...
                if parsed.get("action") == "generate_resume":
                    final_answer = parsed.get(
                        "advice_text", "Here is your edited resume."
                    )
